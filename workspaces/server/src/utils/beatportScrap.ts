...
const artistsScrapper = async (artistsDb: dataDb[]): Promise<Artist[]> => {
  try {
    ...
    const errorMessage = err.response?.status === 404 && "Artist not found on Beatport";

    throw new HttpException(err.response?.status || 500, errorMessage || err.message);
  }
};
...
const labelsScrapper = async (artistsDb: dataDb[]): Promise<Label[]> => {
  try {
    ...
    const err = error as AxiosError;
    const errorMessage = err.response?.status === 404 && "Label not found on Beatport";
